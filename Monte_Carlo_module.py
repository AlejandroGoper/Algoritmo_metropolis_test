...
def displacement_move():
    ...
    vmax_abs = 10 * vmax
    enlo1 = Energia.enerTot(x,0,cajas[0][0])
    enlo2 = Energia.enerTot(x,1,cajas[1][0])
    ...
    v2n = vol1 + vol2 - v1n
    if v1n < vmin or v2n < vmin or v1n > vmax_abs or v2n > vmax_abs: #Si una caja colapso rechazamos
        return cajas[0][0], cajas[1][0],0  
    box1n = v1n**(1/3)
    box2n = v2n**(1/3)
    ...
    en1n = Energia.enerTot(x,0,box1n)
    en2n = Energia.enerTot(x,1,box2n)
    ...
    if np.random.rand() > np.exp(arg1 + arg2):
        ...
        for i in range(npart):
            ...
            x[0][i] *= factor
        return cajas[0][0], cajas[1][0],0# Retorna los valores originales

    return box1n, box2n,1  # Aceptado: retorna nuevos valores
